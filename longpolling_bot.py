...
Этот таскер бот поможет держать твое ментальное состояние в балансе, повышая продуктивность.
...
async def handle_mood_response(user_id: int, text: str):
    ...
    try:
        ...
        if 1 <= mood_level <= 7:
            ...
            if ritual_type == "morning":
                greeting = "Спасибо за ответ! Хорошего продуктивного дня! 🌟"
            else:
                greeting = "Спасибо за ответ! Приятного расслабляющего вечера! 🌙"
            
            await send_message(
                user_id,
                f"✅ Записано: {mood_level} - {mood_name}\n\n"
                f"{greeting}"
            )
            return True
        else:
